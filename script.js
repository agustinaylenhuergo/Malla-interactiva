    ...
    planDeEstudiosData.años.forEach(añoData => {
        ...
        añoData.cuatrimestres.forEach(cuatrimestreData => {
            ...
            cuatrimestreData.materias.forEach(materia => {
                ...
                materiaLi.dataset.codigo = materia.codigo; // Almacena el código para fácil acceso
...
function renderizarConocimientosComunes() {
    planDeEstudiosData.conocimientos_comunes_unlam.forEach(conocimiento => {
        ...
        // Opcional: podrías hacer que estos también muestren detalles si clickean
...
function mostrarDetallesMateria(materiaSeleccionada) {
    ...
    if (materiaSeleccionada.correlatividad) {
        // Manejar correlatividades con '/' para múltiples dependencias
    ...
    // Esto requiere iterar sobre TODAS las materias para ver cuáles la tienen como correlativa
    planDeEstudiosData.años.forEach(año => {
        año.cuatrimestres.forEach(cuatrimestre => {
            cuatrimestre.materias.forEach(materia => {
                // Verificar si la correlatividad de esta materia contiene el código de la materia seleccionada
                if (materia.correlatividad && materia.correlatividad.split('/').includes(materiaSeleccionada.codigo)) {
    ...
    // Similarmente para los conocimientos comunes (si tienen correlatividades en el JSON y quieres que se resalten)
    planDeEstudiosData.conocimientos_comunes_unlam.forEach(conocimiento => {
        if (conocimiento.correlatividad && conocimiento.correlatividad.split('/').includes(materiaSeleccionada.codigo)) {
            const habilitaElement = document.querySelector(`[data-codigo="${conocimiento.codigo}"]`);
            if (habilitaElement) {
                habilitaElement.classList.add('materia-correlativa');
            }
        }
